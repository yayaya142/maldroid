...
def test_each_profile_exposes_only_core_and_its_tools(app_config: AppConfig) -> None:
    ...
    expectations = {
        "native": mcp_tool_name("inspect_elf_file"),
        "flutter": mcp_tool_name("inspect_flutter_artifacts"),
        "unity": mcp_tool_name("detect_unity_backend"),
        "cordova": mcp_tool_name("inspect_cordova_config"),
        "cocos": mcp_tool_name("detect_cocos_script_type"),
    }
...
def test_flutter_inventory_and_unconfigured_blutter(app_config: AppConfig) -> None:
    ...
    inspected = dispatcher.execute(mcp_tool_name("inspect_flutter_artifacts"), {"path": "."})
    assert inspected.data["flutter_aot_indicators"] is True
    availability = dispatcher.execute(mcp_tool_name("check_blutter_availability"), {})
...
def test_unity_cordova_and_cocos_static_detection(app_config: AppConfig) -> None:
    ...
    backend = unity.execute(mcp_tool_name("detect_unity_backend"), {"path": "."})
    ...
    config = cordova.execute(mcp_tool_name("inspect_cordova_config"), {"path": "config.xml"})
    assert config.data["root_attributes"]["id"] == "example"
    plugins = cordova.execute(mcp_tool_name("list_cordova_plugins"), {"path": "."})
    ...
    detected = cocos.execute(mcp_tool_name("detect_cocos_script_type"), {"path": "."})
